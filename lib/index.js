...
*/
function asDomainTableSchema(schema) {
	return schema;
}
/**
...
const teamRoomsDomainSpec = defineDomain({
	...
	version: 1,
	tables: {
		rooms: domainTable(asDomainTableSchema(roomRecordSchema)),
		bus: domainTable(asDomainTableSchema(busMessageSchema)),
		tasks: domainTable(asDomainTableSchema(taskRecordSchema)),
		timeline: domainTable(asDomainTableSchema(timelineEventSchema))
	}
});
//#endregion
...
var RoomHub = class extends Service {
	...
	*/
	hasMember(sessionId) {
		if (this.rooms === void 0) return false;
		for (const [, record] of this.rooms.entries()) if (this.memberOf(record, sessionId) !== void 0) return true;
		return false;
	}
	/**
	...
	*/
	injectBrief(sessionId, room) {
		...
		if (rooms === void 0) this.briefed.set(key, /* @__PURE__ */ new Set([room.roomId]));
...
function apply(ctx, config) {
	...
	ctx.inject(["storageDomain"], (roomCtx) => {
		...
		roomCtx.on("agent/created", (payload) => {
			...
			try {
				if (source === "clear" || source === "compact") return void 0;
				if (!hub.hasMember(agent.id)) return void 0;
				queueMicrotask(() => {
